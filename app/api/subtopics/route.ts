...
export async function POST(request: Request) {
  try {
    ...
    if (!assessment_id || !name?.trim()) {
      return NextResponse.json(
        { success: false, error: 'Assessment ID and name are required' },
        { status: 400 }
      );
    }
    ...
    // Create default rating records for all students in this assessment's class
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to create subtopic' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (isNaN(subtopicIdInt)) {
      return NextResponse.json(
        { success: false, error: 'Invalid subtopic ID' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to delete subtopic' },
      { status: 500 }
    );
  }
}
